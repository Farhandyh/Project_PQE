...
const Sidebar = () => {
  ...
  const SIDEBAR_LINKS = [
    ...
    { id: 7, path: "/dashboard/testingmachine", name: "Testing Machine", icon: FaCarBattery },
    { id: 8, path: "/dashboard/testingmachine", name: "Testing Machine", icon: FaCarBattery },
    { id: 9, path: "/dashboard/testingmachine", name: "Testing Machine", icon: FaCarBattery },
  ];
    ...
    >
          ...
          {SIDEBAR_LINKS.slice(0, 7).map((link, index) => ( // Show only first 6 items
            <li
              ...
              className={`font-medium rounded-md py-2 px-5 hover:bg-white hover:text-red-E01414 ${
                activeLink === index
                  ? "bg-white text-red-E01414 pr-0 rounded-r-none"
                  : ""
              }`}
            >
              <Link
      ...
      <Link to="/login">
        <div className="w-full absolute bottom-5 left-0 px-4 py-2 cursor-pointer text-center">
          <p
            className="flex items-center space-x-2 text-s text-white font-bold py-2 px-5 hover:bg-white rounded-full hover:text-red-E01414"
            path="/login"
          >
            <HiOutlineLogin />
